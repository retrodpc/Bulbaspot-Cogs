...
class SaveyBot:
    ...
    def __init__(self, bot):
        self.bot = bot
        self.save_data = fileIO("data/saveybot/saveybot.json", 'load')

    def save_settings(self):
        fileIO('data/saveybot/saveybot.json', 'save', self.settings)


    @asyncio.coroutine
    def lowest_available():
        save_list = []
        for save_state in self.save_data:
            save_slot = self.save_data[self.save_data.index(save_state)]["slot"]
            if (int(save_slot) > 0):
    ...
    #def fetch_all_slots():
        #slots = []
        #for save_state in self.save_data:
            #save_content = self.save_data[self.save_data.index(save_state)]["slot"]
    ...
    def fetch_all_content():  # returns the content of every goddamned save ever
        content = []
        for save_state in self.save_data:
            save_content = self.save_data[self.save_data.index(save_state)]["message"]
    ...
    def fetch_content(slots: list):  # returns an array full of saves that match the given slots
        content = []
        for slot in slots:
            self.slot_index = yield from self.slot_index(slot)
            save_content = self.save_data[self.slot_index]["message"]
            content.append(save_content)
        return content
    ...
    @asyncio.coroutine
    def owner(self.owner: str, **kwargs):  # using name as well as id to maintain compatibility with irc savestates
        save_list = []
        self.owner_id = kwargs.get('self.owner_id', None)
        for save_state in self.save_data:
            save_slot = self.save_data[self.save_data.index(save_state)]["slot"]
            save_author = self.save_data[self.save_data.index(save_state)]["name"]
            if (save_author.lower() == self.owner.lower()):
                save_list.append(save_slot)
        if self.owner_id is not None:
            for save_state in self.save_data:
                try:
                    saver_id = self.save_data[self.save_data.index(save_state)]["saver_id"]
                    save_slot = self.save_data[self.save_data.index(save_state)]["slot"]
                    if (saver_id == self.owner_id):
                        save_list.append(save_slot)
                except:
                    pass
    ...
    def save(self, ctx, *, params: str = None):
        ...
        if params is None:
            ...
            slot = yield from self.lowest_available()
            content = params
        ...
        try:  # seeing if the savestate exists
            data_index = yield from self.slot_index(slot)
        except Exception:
            if is_int(slot):
                self.save_data.append({"name": ctx.message.author.name, "slot": slot, "message": content, "saver_id": ctx.message.author.id})
                save(self.save_data)
        ...
        try:  # seeing if the savestate has an author.id saved
            saver_id = self.save_data[data_index]["saver_id"]
        except:
            saver_id = "Unknown"

        if (ctx.message.author.id == saver_id):
            self.save_data[data_index] = ({"name": ctx.message.author.name, "slot": slot, "message": content, "saver_id": ctx.message.author.id})
            save(self.save_data)
    ...
    def low(self):
        ...
        lowest = yield from self.lowest_available()
    ...
    def load(self, slot: str):
        ...
        state_content = self.save_data[data_index]["message"]
    ...
    def road(self, ctx, user: str = None):
        ...
        if user is None:
            random_index = randint(0,len(self.save_data)-1)
            state_content = self.save_data[random_index]["message"]
            state_slot = self.save_data[random_index]["slot"]
            state_author = self.save_data[random_index]["name"]
            ...
            slots = yield from self.owner(user_name, self.owner_id=user_id)

            random_slot = slots[randint(0,len(slots)-1)]
            random_self.slot_index = yield from self.slot_index(random_slot)
            state_content = self.save_data[random_self.slot_index]["message"]
            state_slot = random_slot
            state_author = self.save_data[random_self.slot_index]["name"]
            yield from self.bot.say('~ {1} ({2}): {0}'.format(state_content, state_slot, state_author))
    ...
    def whois(self, slot: str):
        ...
        state_author = self.save_data[data_index]["name"]
    ...
    def search(self, ctx, *, search: str):
        ...
        search_slots = yield from self.search_content(search)  # query
        if len(search_slots) > 0:
            ...
            if (len(output) <= 2000):
                ...
                with open("./temp/" + filename, "w") as output_file:
                    output_file.write(output)
                with open("./temp/" + filename, "r") as output_file:
    ...
    def loaduser(self, ctx, *, user: str):
        ...
        saves = yield from self.owner(user_name, self.owner_id=user_id)
        if len(saves) > 0:
    ...
    def markov(self, ctx, *args):
        ...
        if user is None:
            all_content = yield from self.fetch_all_content()
            all_words = yield from self.array_word_split(all_content)
            random_words = yield from self.get_random(all_words, count)
            ...
            slots = yield from self.owner(user_name, self.owner_id=user_id)
            slots_content = yield from self.fetch_content(slots)
            all_words = yield from self.array_word_split(slots_content)
            random_words = yield from self.get_random(all_words, 10)
            content = ' '.join(random_words)
            if len(content) > 1998:
                content = content[:1998]
            yield from self.bot.say("~ " + content)
        ...
        if current.keys() != default.keys():
            for key in default.keys():
                if key not in current.keys():
                    ...
                    print(
                        "Adding " + str(key) + " field to saveybot.json")

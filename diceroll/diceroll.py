# Port of Chaithi's dice rolling bot.
...
class Dice:
    """A modular port of Chaithi's dice roller."""
    ...
    @commands.command(pass_context=True,
        aliases=["rollbetter","rollalt","roll_alt","diceroll","dice_roll"])
    @asyncio.coroutine
    def roll_better(self, ctx, roll : str):
        ...
        try:
            if (roll is None):
                raise ValueError("Add an argument dammit!")
            if (modifier != 0):
            ...
            if a != 0 and b != 0:
                yield from self.bot.say("{} rolls {}-{}. Result: {}".format(author, a, b, roll_basic(a, b, modifier, threshold)))
            else:
                yield from self.bot.say("{} rolls {}d{}. Results: {}".format(author, num_of_dice, dice_type, roll_hit(num_of_dice, dice_type, hit, modifier, threshold)))
